...
Command examples:
    ...
    python 09_bayesian_gnn_re.py --meta_df 'where/your/metacsv/is' --graphs_root 'where/your/graphs/are'
...
def collate_graphs(graphs):
    ...
    if len(eas)>0 and any([e.numel()>0 for e in eas]):
        batch_ea = torch.cat([e if e.numel()>0 else torch.empty((0, eas[0].shape[1] if hasattr(eas[0],'ndim') and eas[0].ndim>1 else 0)) f>
    else:
...
class GNNMember(nn.Module):
    def __init__(self, in_dim, hidden_dim=128, num_layers=3, dropout=0.2, use_pyg=USE_PYG):
        super().__init__()
    ...
    def forward(self, x, edge_index, batch_vec, edge_attr=None):
        if self.use_pyg:
            ...
            h_nodes = self.mlp_node(x) if x.numel()>0 else torch.zeros((batch_vec.shape[0] if batch_vec is not None else 1, self.hidden_di>
            if batch_vec is None or batch_vec.numel()==0:
...
def build_ensemble_from_specs(ensemble_specs, in_dim, device, hidden_dim=128, num_layers=3, dropout=0.2):
    ...
    for spec in ensemble_specs:
        ...
        print(f"  -> ensemble model '{spec.get('name')}' created")
...
def main(args):
    ...
    train_list = [data_list[i] for i in train_idx]; val_list = [data_list[i] for i in val_idx]; test_list = [data_list[i] for i in test_id>
    ...
    model = GNNMember(in_dim, hidden_dim=args.hidden_dim, num_layers=args.num_layers, dropout=args.dropout)
    model.to(device)
    ...
    # training: store only epoch mean loss
    epoch_logs=[]
    val_logs=[]
    for ep in range(1, args.epochs+1):
        ...
        epoch_logs.append({'epoch': ep, 'mean_loss': epoch_mean_loss})
        # validation (gnina mse original scale)
        model.eval()
        with torch.no_grad():
            preds=[]; ys=[]
            ...
            if len(preds)>0:
                ...
                val_mse = float('nan')
        val_logs.append({'epoch': ep, 'val_mse': val_mse})
        t1=time.time()
    ...
    # save epoch-level train loss and val_mse
    pd.DataFrame(epoch_logs).to_csv(out_dir / "train_epoch_losses.csv", index=False)
    pd.DataFrame(val_logs).to_csv(out_dir / "train_val_mse.csv", index=False)
    print("[info] saved epoch logs")
    ...
    # optional: initialize ensemble from trained model
    if args.init_ensemble_from_trained:
        trained_state = model.state_dict()
        for em in ensemble_models:
            try: em.load_state_dict(trained_state)
            except Exception: pass

    print(f"[info] sampling ensemble: M={len(ensemble_models)}, mc_T={args.mc_T}, enable_mc_dropout={args.enable_mc_dropout}")
    S_samples = sample_ensemble_predictions(ensemble_models, test_loader, device, mc_T=args.mc_T, enable_mc=args.enable_mc_dropout, verbos>
    if S_samples.size==0:
    ...
    rank_confidence = 1.0 - (mean_rank / (max(1, N-1)))  # 0..1 high->consistently high rank

    # assemble true values and final DataFrame (respecting test_list order)
    ligand_ids=[]; smiles_list=[]; true_gnina=[]; true_vina=[]
    for d in test_list:
        y = d.get('y')
        if isinstance(y, torch.Tensor) and y.numel()>=2:
            gn = float(y[0].item()) if not np.isnan(y[0].item()) else float('nan')
            vi = float(y[1].item()) if not np.isnan(y[1].item()) else float('nan')
        else:
            gn = float('nan'); vi = float('nan')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--meta_csv', type=str, default='/home/ssm-user/project/scores/processed_graphs.csv', required=True)
    parser.add_argument('--graphs_root', type=str, default='/home/ssm-user/project/graphs', required=True)
