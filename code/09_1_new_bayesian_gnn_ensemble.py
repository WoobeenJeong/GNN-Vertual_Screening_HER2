...
def load_graphs_from_meta(meta_csv, graphs_root=None, target_cols=('gnina_affinity','vina_affinity'), replace_nan_with=None):
    ...
    for _, row in df.iterrows():
        ...
        global_attr = g.get('global_attr', None)
        ...
        data_list.append({
            'x': x, 'edge_index': edge_index, 'edge_attr': edge_attr, 'global_attr': global_attr, 
            'y': y, 'smiles': g.get('smiles'), 'ligand_id': g.get('ligand_id'), 'graph_path': gpath
        })
    return data_list
...
def collate_graphs(graphs):
    xs, eis, eas, gas, ys, node_counts, ligand_ids, smiles = [], [], [], [], [], [], [], []
    node_offset = 0

    default_ga_size = next((g['global_attr'].shape[0] for g in graphs if g.get('global_attr') is not None and hasattr(g.get('global_attr'), 'shape')), 0)
    ...
    for g in graphs:
        ...
        ga = g.get('global_attr')
        if ga is None: 
            ga = torch.zeros(default_ga_size, dtype=torch.float)
        gas.append(ga)
    ...
    batch_ga = torch.stack(gas, dim=0) if gas and any(t is not None and t.numel() > 0 for t in gas) else torch.empty(0)
    ...
    return {
        'x': batch_x, 'edge_index': batch_ei, 'edge_attr': batch_ea, 'global_attr': batch_ga,
        'y': batch_y, 'node_counts': node_counts, 'batch_vec': batch_vec, 
...
class GNNMember(nn.Module):
    def __init__(self, in_dim, hidden_dim, num_layers, dropout, conv_type='gcn', edge_dim=None, global_dim=None, use_pyg=USE_PYG):
        super().__init__()
        ...
        self.has_global = global_dim is not None and global_dim > 0
        if self.has_global:
            combined_dim += global_dim
            self.global_mlp = nn.Sequential(nn.Linear(global_dim, global_dim), nn.SiLU(), nn.Dropout(dropout))

        self.head_gnina = nn.Sequential(nn.Linear(combined_dim, hidden_dim//2), nn.SiLU(), nn.Linear(hidden_dim//2, 1))
        self.head_vina = nn.Sequential(nn.Linear(combined_dim, hidden_dim//2), nn.SiLU(), nn.Linear(hidden_dim//2, 1))

    def forward(self, x, edge_index, batch_vec, edge_attr=None, global_attr=None):
        ...
        else: # Fallback for non-PyG environment
        ...
        if self.has_global and global_attr is not None and global_attr.shape[0] == hg.shape[0]:
            global_features = self.global_mlp(global_attr)
            combined_features = torch.cat([hg, global_features], dim=1)
        else:
            combined_features = hg
            
        out1 = self.head_gnina(combined_features).view(-1)
        out2 = self.head_vina(combined_features).view(-1)
...
class CCCLoss(nn.Module):
    ...
    def forward(self, preds, y):
        y_mean = torch.mean(y); preds_mean = torch.mean(preds)
        y_var = torch.var(y); preds_var = torch.var(preds)
...
def batch_to_tensors_for_model(batch, device):
    ...
    global_attr = batch.get('global_attr').to(device)
    y = batch['y'].to(device)
  
    return x, edge_index, batch_vec, edge_attr, global_attr, y

def train_one_epoch_log(model, optimizer, train_loader, device, criterion):
    model.train()
    batch_losses = []
    for batch in train_loader:
        x, edge_index, batch_vec, edge_attr, global_attr, y = batch_to_tensors_for_model(batch, device)
        optimizer.zero_grad()
        preds = model(x, edge_index, batch_vec, edge_attr, global_attr)
...
def build_ensemble_from_specs(specs, in_dim, device, hidden_dim, num_layers, dropout, edge_dim, global_dim):
    models = []
    for spec in specs:
        ...
        m = GNNMember(in_dim, hidden_dim=hidden_dim, num_layers=num_layers, dropout=dropout, conv_type=conv_type, edge_dim=edge_dim, global_dim=global_dim)
        m.to(device)
        models.append(m)
    return models
        ...
        for draw in range(num_draws):
            ...
            with torch.no_grad():
                for batch in batches:
                    x, edge_index, batch_vec, edge_attr, global_attr, _ = batch_to_tensors_for_model(batch, device)
                    out = m(x, edge_index, batch_vec, edge_attr, global_attr)
...
def main(args):
    ...
    global_dim = next((d['global_attr'].shape[0] for d in data_list if d.get('global_attr') is not None and hasattr(d.get('global_attr'), 'shape')), None)
    ...
    print(f"[info] Determined global feature dimension (global_dim): {global_dim}")
    ...
    ensemble_models = build_ensemble_from_specs(ensemble_specs, in_dim, device, args.hidden_dim, args.num_layers, args.dropout, edge_dim=edge_dim, global_dim=global_dim)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--graphs_root', type=str, default='/home/ssm-user/project/scores/graphs', help="Directory containing the graph .pt files.")
